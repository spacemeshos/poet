...
func validateChallenge(ctx context.Context, challenge *shared.Challenge, atxs types.AtxProvider, postConfig *types.PostConfig, proofVerifier func(*postShared.Proof, *postShared.ProofMetadata) error) error {
	if initialPost := challenge.InitialPost; initialPost != nil {
		...
		if meta.BitsPerLabel != postConfig.BitsPerLabel {
			return fmt.Errorf("BitsPerLabel mismatch: challenge: %d, config: %d", meta.BitsPerLabel, postConfig.BitsPerLabel)
		}
		if meta.LabelsPerUnit != postConfig.LabelsPerUnit {
			return fmt.Errorf("LabelsPerUnit mismatch: challenge: %d, config: %d", meta.LabelsPerUnit, postConfig.LabelsPerUnit)
		}
		if meta.K1 != postConfig.K1 {
			return fmt.Errorf("K1 mismatch: challenge: %d, config: %d", meta.K1, postConfig.K1)
		}
		if meta.K2 != postConfig.K2 {
			return fmt.Errorf("K2 mismatch: challenge: %d, config: %d", meta.K2, postConfig.K2)
		}
		if meta.NumUnits < postConfig.MinNumUnits || meta.NumUnits > postConfig.MaxNumUnits {
			return fmt.Errorf("NumUnits not in the required range [%d <= %d <= %d]", postConfig.MinNumUnits, meta.NumUnits, postConfig.MaxNumUnits)
		}
	...
	// Verify against previous ATX
	{
		...
		if err != nil {
			return fmt.Errorf("failed to fetch previous ATX (%w)", err)
		}
		if bytes.Compare(challenge.NodeID, atx.NodeID) != 0 {
			return fmt.Errorf("NodeID mismtach: challenge: %X, previous ATX: %X", challenge.NodeID, atx.NodeID)
		}
		if challenge.PubLayerId != atx.PubLayerID+1 {
			return fmt.Errorf("Publayer ID mismatch: challenge publayer ID: %d, previous ATX publayer ID: %d", challenge.PubLayerId, atx.PubLayerID)
		}
	}
	// Verify against positioning ATX
	{
		...
		if err != nil {
			return fmt.Errorf("failed to fetch positioning ATX (%w)", err)
		}
		if challenge.PubLayerId != atx.PubLayerID+1 {
			return fmt.Errorf("Publayer ID mismatch: challenge publayer ID: %d, positioning ATX publayer ID: %d", challenge.PubLayerId, atx.PubLayerID)
		}
	}
	return nil
}
