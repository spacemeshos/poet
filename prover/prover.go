...
func makeRecoveryProofTree(
	...
	for layer, file := range layersFiles {
		...
		if expectedWidth < width {
			...
			logging.FromContext(context.Background()).Sugar().Infof("Recovery: layer %v cache file width is ahead of the last known merkle tree state. expected: %d, found: %d. Truncating file...", layer, expectedWidth, width)
